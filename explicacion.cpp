...
/*
Primero, creamos un proyecto de aplicacion de consola, para separar bibliotecas creando un archico h y otro cpp
*/
...
int sumaDeDados(int dados[], int cantidad, int primerBloqueador, int segundoBloqueador){
    ...
    for (int i=0; i<cantidad; i++){
        ...
        if (dados[i] != primerBloqueador && dados[i] != segundoBloqueador && dados[i] == dados[0]){
        dadosIguales = true;
        }
        if (dadosIguales) {
        suma *= 2;
